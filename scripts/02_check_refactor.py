...
# ---------- Utilidades ----------
def find_first_package() -> Optional[str]:
    candidates = [p for p in glob.glob("src/*") if os.path.isdir(p)]
...
def check_functions_responsibility(py_file: str, logical_name: str) -> Tuple[bool, str]:
    ...
    # regla: al menos una función que match en cada grupo (si el grupo no está vacío)
    for group in rules:
        if not group:  # grupo vacío => no aplica esa parte
            continue
        if not any(name_matches_any(fn, group) for fn in public):
...
def main():
    pkg = find_first_package() or "<package_name>"
    # 1) Módulos
    ...
    # 3) POO sugerida (clases)
    print_header("POO sugerida (clases reutilizables)")
    oop_hits = []
    for logical, paths in modules.items():
        for p in paths:
            ok, hits = check_oop_suggested(p)
            if hits:
                oop_hits.extend([(p, h) for h in hits])
    if oop_hits:
        for p, h in oop_hits:
    ...
    print(f"{'tests/test_*.py':30}{state_tag(has_tests, REQUIRED)}   {', '.join(map(os.path.relpath, test_files)) if test_files else 'No hay tests'}")
    if not has_tests:
        any_missing_required = True
    ...
    print(f"{'Docstrings funciones (>=50%)':30}{state_tag(doc_ok, RECOMMENDED)}   ratio={q['doc_ratio']:.2f} ({q['n_funcs']} funcs)")
    print(f"{'Type hints funciones (>=30%)':30}{state_tag(ann_ok, RECOMMENDED)}   ratio={q['ann_ratio']:.2f}")

    need_main_guard = check_main_guards(py_files)
    if need_main_guard:
        for f in need_main_guard:
            print(f"{'__main__ en scripts':30}{state_tag(False, RECOMMENDED)}   Falta guardia en {os.path.relpath(f)}")
    else:
        print(f"{'__main__ en scripts':30}{state_tag(True, RECOMMENDED)}   OK")
    ...
    # 7) Sugerencias rápidas
    print("\nSugerencias:")
    if not present_pre:
        print(" - Crea src/<paquete>/preprocess.py (carga/limpieza/features básicas).")
    if not present_train:
        print(" - Crea src/<paquete>/train.py (entrenamiento; registra métricas/artefactos).")
    if not present_eval:
        print(" - Crea src/<paquete>/evaluate.py (evaluación; reporte de métricas).")
    if not has_tests:
